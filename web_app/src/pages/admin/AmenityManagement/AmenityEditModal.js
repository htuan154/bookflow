// src/pages/admin/AmenityManagement/AmenityEditModal.js
import React, { useState, useEffect } from 'react';
import { X } from 'lucide-react';
import useAmenity from '../../../hooks/useAmenity';

const AmenityEditModal = ({ amenity, onClose, onSuccess }) => {
  const { updateAmenity, localLoading } = useAmenity();
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg max-w-md w-full">
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-200">
          <h3 className="text-xl font-bold text-gray-800">Chỉnh sửa tiện nghi</h3>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-4">
          ...
          <div className="flex gap-3 justify-end mt-6 pt-4 border-t border-gray-200">
            ...
            <button
              ...
              disabled={localLoading}
            >
              {localLoading ? 'Đang lưu...' : 'Lưu thay đổi'}
            </button>
          </div>
        </form>
      </div>
